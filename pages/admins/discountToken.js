...
import { DiscountTable } from "../../components/couponsAdmin"
import { createAccountDiscountCoupon, fetchAllAccountDiscountCoupons} from "../../src/services/admin";
...
function AllAccountDiscountCoupons() {
  ...
  const getCoupons = async () => {
    ...
    const response = await  fetchAllAccountDiscountCoupons(token);
...
const handleSubmit = async () => {
  ...
  if (validateForm()) {
    ...
    setMsg('');
    setProgress(true)
    const response = await createAccountDiscountCoupon(token,formData);
    console.log(response)
  ...
  return (
    <div>
      <AdminAndCsoDashBoard>
        ...
        <DiscountTable records={listDiscount} setItem={setListDiscount} callback={fetchAllAccountDiscountCoupons}/>
      </AdminAndCsoDashBoard>
    </div>
  );
}

export default AllAccountDiscountCoupons;
