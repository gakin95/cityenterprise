...
import { deleteDiscountToken } from "../src/services/eventServices";
import TokenDialogue from "../components/dialogue/token";
import { MyConfirmationDialog, MyCustomButton } from "../common";
...
export default function CustomizedTables({ records, setItem, callback }) {
  ...
  const onDelete = async (id) => {
    ...
    setItem(curItem);
    const response = await deleteDiscountToken(token, id);
              ...
              <TableCell>
                <div className={classes.btnicon}>
                  <EditIcon
                    ...
                    onClick={() =>
                      router.push(`/stakeholders/token/${item.id}`)
                    }
                  />
